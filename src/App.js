...
class App extends Component {
        ...
        axios.get(back_api)
            .then((response)=>{
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    groups[obj].push({value:data[i].price,timestamp:new Date(data[i].timestamp)});
                }
                console.log(groups);
    ...
    updateSelectedPoints(cur,radius){
        ...
        const plot2Graph = this.state.coords.filter((coord,index)=> {
            ...
            const isBetweenDateRange = !startDate || !endDate || moment(coord.timestamp).isBetween(startDate, endDate);
    ...
    render() {
        ...
        return (
            ...
            <Gmaps
                ...
                // const shouldDisplay = isSelected||isBetweenDateRange;
                ...
                return (
                    ...
                    <Marker
                        ...
                        lng={coord.geo.lng}
                        draggable={false}
                        onClick={()=>{console.log(index); console.log(coord); this.setState({selectedPlace:index,selectedPlaceObj:coord}); this.updateSelectedPoints(coord,this.state.radius)}}
                        ...
                        lat={coord.geo.lat}
                        lng={coord.geo.lng}
                        content={`<div><img height="32px" width="32px" style="border-radius:50%;" src=${coord.profileImg} /><br/> Poster: ${coord.poster} <br/>Price:  ฿${lastPrice.value}<br/> Date: ${moment(lastPrice.timestamp).format("DD-MM-YYYY h:mm:ss")}<br/> </div>`}
                        width={'30px'}
                                ...
                                datasets: [
                                    {
                                        ...
                                        data: data.map((price, index) => {
                                            console.log(price);
                                            return {x: price.timestamp, y: price.value}
                                        })
